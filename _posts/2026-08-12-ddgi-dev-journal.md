...
By leveraging Vulkan’s acceleration structures and ray query shader functions, I can get a clean representation of light visibility from any point in the scene. First, build an acceleration structure out of the scene. The table below shows mapping between glTF concepts, engine objects, and Vulkan acceleration structures

|-|-|-|
|glTF|My Engine|BLAS|TLAS|
|mesh|renderable|geometry|--|
|node|scene node|BLAS|instance|
|scene|scene|--|TLAS|


Image on the left shows AABB overlap. Image on the right is a traversal timing heatmap. Both views are captured with NSight Graphics.

<p align="center">
  <img src="/images/DDGI/as_aabb_overlap.png" alt="ray hit radiance" width="45%">
  <img src="/images/DDGI/as_traversal.png" alt="ray hit positions" width="45%">
</p>

Non-opaque primitives, in this case alpha masked foliage, are the most expensive to traverse because intersections with them are often not committed. Traversal must continue through multiple non-opaque intersections until the ray hits a nearby opaque wall. 

Another problem is poor AABB geometry grouping. The flowerpots, pillars, and curtains on the lower level are grouped into large meshes whose parts share the same materials. These meshes contain large empty spaces. Their AABBs therefore overlap significantly, making the BLAS less compact and less efficient to traverse.

The solution is simple: separate the meshes. The image on the left shows less AABB overlap, while the image on the right shows faster traversal, if you look hard enough. Nsight measurements from the same viewpoint show that the ray tracing time dropped from 4.5 ms to 3.8 ms.

<p align="center">
  <img src="/images/DDGI/as_aabb_overlap_optimized.png" alt="ray hit radiance" width="45%">
  <img src="/images/DDGI/as_traversal_optimized.png" alt="ray hit positions" width="45%">
</p>

The image below was generated by tracing rays from the camera in the direction of each screen pixel. Only direct lighting with a Lambertian diffuse BRDF is accounted for. This is often enough for indirect lighting unless you’re aiming for sharp caustics reflected off metallic surfaces.
